...
import com.myee.tarot.configuration.type.*;
 ...
 */
@Controller
public class ReceiptPrintedController {

    private static final Logger LOGGER = LoggerFactory.getLogger(ReceiptPrintedController.class);
    ...
    @ResponseBody
    public AjaxPageableResponse displayDashboard(WhereRequest whereRequest, HttpServletRequest request) {
        ...
        try {
            ...
            PageResult<ReceiptPrinted> receiptPrintedPageResult = receiptPrintedService.listByMerchantStoreId(whereRequest, merchantStore1.getId());
    ...
    @RequestMapping(value = {"admin/configuration/receiptPrinted/listModuleType"}, method = RequestMethod.GET)
    @ResponseBody
    public List listType() {
        return new ReceiptPrintedModuleType().getReceiptPrintedModuleType4Select();
    }

    @RequestMapping(value = {"admin/configuration/receiptPrinted/listModuleItemType"}, method = RequestMethod.GET)
    @ResponseBody
    public List listItemType() {
        return new ReceiptPrintedItemType().getReceiptPrintedItemType4Select();
    }

    @RequestMapping(value = {"admin/configuration/receiptPrinted/listModuleItemFontType"}, method = RequestMethod.GET)
    @ResponseBody
    public List listItemFontType() {
        return new ReceiptPrintedItemFontType().getReceiptPrintedItemFontType4Select();
    }

    @RequestMapping(value = {"admin/configuration/receiptPrinted/listModuleItemSizeType"}, method = RequestMethod.GET)
    @ResponseBody
    public List listItemSizeType() {
        return new ReceiptPrintedItemSizeType().getReceiptPrintedItemSizeType4Select();
    }

    @RequestMapping(value = {"admin/configuration/receiptPrinted/listModuleItemQrSizeType"}, method = RequestMethod.GET)
    @ResponseBody
    public List listItemQrSizeType() {
        return new ReceiptPrintedItemQrSizeType().getReceiptPrintedItemQrSizeType4Select();
    }
    ...
    public AjaxResponse deviceUsedBindProductUsed(@RequestParam(value = "pushString") String pushString, @RequestParam(value = "receiptPrintedId") Long receiptPrintedId, HttpServletRequest request) {
        ...
        moduleContent += "<" + (receiptPrinted.getReceiptType().toLowerCase()) + ">\n";
        for (int i = 0; i < list.size(); i++) {
            moduleContent += "<" + (list.get(i).getItemType().toLowerCase()) +
                    ...
                    "</" + (list.get(i).getItemType().toLowerCase()) + ">\n";
        }
        moduleContent += "</" + (receiptPrinted.getReceiptType().toLowerCase()) + ">";
...
//                    //同一类型下同一设备的绑定关系只能有一条记录，自研平板类型除外
    ...
    private Map<String, Object> objectToEntry(ReceiptPrinted receiptPrinted) {
        ...
        entry.put("receiptTypeText", new ReceiptPrintedModuleType().getReceiptPrintedModuleTypeName(receiptPrinted.getReceiptType()));
